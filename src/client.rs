...
impl<C> PusherBuilder<C> {
  ...
  pub fn new_with_client(http_client: Client<C>, app_id: &str, key: &str, secret: &str) -> PusherBuilder<C> {
    PusherBuilder {
...
mod tests {
  ...
  fn test_private_channel_authentication(){
    let pusher = PusherBuilder::new("id", "278d425bdf160c739803", "7ad3773142a6692b25b8").finalize();
  ...
  fn test_presence_channel_authentication(){
    ...
    let result_decoded : HashMap<String, String> = serde_json::from_str(&result_json.unwrap()).unwrap();
